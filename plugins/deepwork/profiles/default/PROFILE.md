...
- `references/tool-reference.md` — explicit Agent/TaskCreate/TaskUpdate/TaskList/TaskGet/SendMessage/AskUserQuestion/ExitPlanMode syntax (Claude Code does not natively know these — read carefully)
...
8. **Spawn all teammates in parallel** via a single message containing multiple Agent tool calls (one per role). There is no TeamCreate call — the implicit team forms automatically when the first teammate is spawned. Each spawn passes the fully-rendered role template in `prompt:` — resolve the HARD_GUARDRAILS, SOURCE_OF_TRUTH, ANCHORS, WRITTEN_BAR, and TEAM_ROSTER template slots from state.json values before calling. Do NOT pass a `team_name:` argument to Agent — it is accepted but ignored by the CLI (team_name is session-derived and @deprecated; discovery still matches because setup derives the same name). For CRITIC and REFRAMER, include their invariant stance text from `references/critic-stance.md` / `references/reframer-stance.md` verbatim.
...
## 8. Artifact frontmatter templates

Every `.md` artifact written inside the active instance dir MUST carry YAML frontmatter per the schema below. [hooks/frontmatter-gate.sh](../../hooks/frontmatter-gate.sh) enforces the floor fields (`artifact_type`, `author`, `instance`, `task_id` or `task_ids`, `sources`, `delta_from_prior` where applicable) at `PreToolUse:Write|Edit` time. Carve-outs: `log.md`, `prompt.md`, `adversarial-tests.md`, `adversarial-tests-*.md`. Full schema reference: [references/frontmatter-schemas.md](../../references/frontmatter-schemas.md). Field classification: **[L]** = load-bearing (hook-enforced OR skill-consumed as required); **[C]** = cosmetic (skill uses if present).

For the full schemas (critique, findings, coverage, mechanism, reframe, empirical_results, gate-list), see [artifact-schemas.md](artifact-schemas.md). Roles read this file lazily when authoring artifacts.
