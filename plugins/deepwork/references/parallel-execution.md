...
`cwd` on `Agent` is the intended spawn-time primitive (CC source line 399540), but as of CC 2.1.120+ it is not exposed in the public schema and does not take effect. Use the prompt-based `cd` pattern until `cwd` is surfaced publicly.
