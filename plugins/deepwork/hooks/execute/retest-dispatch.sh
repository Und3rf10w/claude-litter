#!/bin/bash
# retest-dispatch.sh — PostToolUse(Write|Edit) advisory async retest dispatch.
#
# After a file write/edit, this hook looks up the test_manifest to find a covering
# test for the written file and dispatches it in the background. Results are written
...
# test_manifest entries: {"source_file": "/abs/path/file.ts", "test_command": "npm test -- src/file.test.ts"}
