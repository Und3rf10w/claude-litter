...
add_member() {
    ...
    jq --arg id "$agent_id" \
       ...
       '.members += [{"agentId": $id, "name": $name, "type": $type, "color": $color}]' \
...
spawn_teammate_tmux() {
    ...
    # Add to team config
    add_member "$team_name" "$agent_id" "$agent_name" "$agent_type"
...
spawn_teammate_kitty() {
    ...
    # Use printf to properly escape the text and send Enter key
    kitten_cmd send-text --match "var:${swarm_var}" -- "$initial_prompt"
    sleep 0.5
    kitten_cmd send-text --match "var:${swarm_var}" $'\n'
...
cleanup_team() {
    ...
    case "$SWARM_MULTIPLEXER" in
        kitty)
            # Kill all kitty windows for this team using user variable
            for agent in $(kitten_cmd ls 2>/dev/null | jq -r --arg team "$team_name" '.[].tabs[].windows[] | select(.user_vars.swarm_team == $team) | .user_vars.swarm_agent' 2>/dev/null); do
                ...
                echo -e "${YELLOW}Closed kitty window: ${agent}${NC}"
            done
            ;;
        tmux)
            ...
            for session in $(tmux list-sessions -F '#{session_name}' 2>/dev/null | grep "swarm-${team_name}"); do
                ...
                echo -e "${YELLOW}Killed tmux session: ${session}${NC}"
            done
            ;;
    esac
    ...
    echo -e "${GREEN}Cleanup complete for team '${team_name}'${NC}"
}
