...
class SwarmPanel(Widget):
    ...
    def _show_empty(self) -> None:
        try:
            ...
            for wid in ("swarm-status-overview", "swarm-progress", "swarm-goal",
                        "swarm-meta", "swarm-heartbeat", "swarm-warnings", "swarm-profile-extras"):
    ...
    def _render_state(self, state: SwarmState) -> None:
        ...
        # Log and Progress tabs
        instance_dir = getattr(state, "instance_dir", None)
        self._render_log(instance_dir)
        self._render_progress_timeline(instance_dir)

    def _render_log(self, instance_dir: Path | None) -> None:
        try:
        ...
        if instance_dir is None:
            scroll.mount(Static("[dim]No instance directory[/dim]", classes="swarm-log-line"))
            return
        log_path = instance_dir / "log.md"
        if not log_path.exists():
            scroll.mount(Static("[dim]No log.md found[/dim]", classes="swarm-log-line"))
            return
        try:
            lines = log_path.read_text(encoding="utf-8").splitlines()
        except Exception:
            scroll.mount(Static("[dim]Error reading log[/dim]", classes="swarm-log-line"))
            return
        if not lines:
    ...
    def _render_progress_timeline(self, instance_dir: Path | None) -> None:
        try:
        ...
        scroll.remove_children()
        if instance_dir is None:
            scroll.mount(Static("[dim]No instance directory[/dim]", classes="swarm-progress-line"))
            return
        progress_path = instance_dir / "progress.jsonl"
        if not progress_path.exists():
            scroll.mount(Static("[dim]No progress.jsonl found[/dim]", classes="swarm-progress-line"))
            return
        try:
            raw_lines = progress_path.read_text(encoding="utf-8").splitlines()
        except Exception:
            scroll.mount(Static("[dim]Error reading progress[/dim]", classes="swarm-progress-line"))
            return
        entries = []
